...
class Cutscene:
    def __init__(self, screen):
        self.screen = screen
        self.video = VideoFileClip("cutscenes/tqt3y-official-cutscene.mp4")
    ...
    def run(self, events, elapsed_time):
        if self.playing:
            ...
            frame = self.video.get_frame(self.current_frame / 96)
            ...
            if self.current_frame >= 100 * self.video.duration + 40:
                self.playing = False
                if self.audio_channel:
                    self.audio_channel.stop()  # Stop the audio
